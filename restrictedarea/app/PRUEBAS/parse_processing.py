...
def parse_polygons(zone_restricted_str):
    """
    Convierte el string zone_restricted en una lista de polígonos válidos para OpenCV.
    Ejemplo de entrada: 
    "{'1':[[(1677,564),(855,1080),...],[(1536,621),(1638,558),...]]}"
    """
    try:
        # Paso 1: Limpiar y convertir a dict
        data = ast.literal_eval(zone_restricted_str.replace("'", '"'))
        
        # Paso 2: Extraer todos los polígonos (incluso si están anidados bajo claves)
        polygons = []
        if isinstance(data, dict):
            for key in data:
                if isinstance(data[key], list):
                    polygons.extend(data[key])
        elif isinstance(data, list):
            polygons = data
        else:
            raise ValueError("Formato no soportado")
        
        # Paso 3: Convertir strings "(x,y)" a tuples (x, y)
        parsed_polygons = []
        for polygon in polygons:
            ...
            if len(parsed_points) >= 3:  # Mínimo 3 puntos para un polígono
...
def get_points_from_camera_id(data,cam_id):
    try:
        ...
        return parse_polygons(zone_restricted_str)
