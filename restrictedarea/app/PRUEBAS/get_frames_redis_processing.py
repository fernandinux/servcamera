...
import json
import os
import cv2  # Añadido para el manejo de imágenes
from config import *
...
# INICIO - CÓDIGO TEMPORAL PARA GUARDAR IMÁGENES
# Directorio donde se guardarán las imágenes
IMAGES_DIR = "camera_frames"
# Crear directorio si no existe
if not os.path.exists(IMAGES_DIR):
    os.makedirs(IMAGES_DIR)
# Registro de cámaras ya procesadas para evitar guardar múltiples imágenes
...
def save_camera_frame(data):
    """Guarda el frame de la cámara en el directorio designado"""
    try:
        camera_id = data.get(DATA_CAMERA)
        
        # Si ya procesamos esta cámara, no guardar otra imagen
        ...
        if frame is not None:
            ...
            filename = f"{IMAGES_DIR}/camera_{camera_id}_{int(time.time())}.jpg"
            # Guardar imagen
            cv2.imwrite(filename, frame)
            # Marcar cámara como procesada
            processed_cameras.add(camera_id)
            logger.info(f"Imagen de cámara {camera_id} guardada en {filename}")
    except Exception as e:
        logger.error(f"Error al guardar imagen de cámara {camera_id}: {str(e)}")
# FIN - CÓDIGO TEMPORAL PARA GUARDAR IMÁGENES
...
def procesamiento_coordenadas_sync(data, publish_callback):
    ...
    try:
        # CÓDIGO TEMPORAL: Guardar imagen de la cámara
        save_camera_frame(data)
