import time
import numpy as np
import redis
import ast
import re
import json
import cv2
import base64
from datetime import datetime
import os
from config import *
from config_logger import configurar_logger


logger = configurar_logger(name=NAME_LOGS, log_dir=LOG_DIR, nivel=LOGGING_CONFIG)

# Directorio para guardar las imágenes de debug
DEBUG_DIR = "./debug_images" 
os.makedirs(DEBUG_DIR, exist_ok=True)

# Diccionario para almacenar el tiempo de entrada
vehicle_timers = {}

# Pool de conexiones Redis
redis_pool = redis.ConnectionPool(
    host=REDIS_HOST, 
    port=REDIS_PORT, 
    db=REDIS_DB,
    socket_timeout=REDIS_TIMEOUT
)

# Parámetros ajustables para la posición del punto de detección
POINT_HORIZONTAL_OFFSET = 0.5  # 20% desde los bordes horizontales (ahora controla el centro)
POINT_VERTICAL_POSITION = 0.7   # 70% desde la parte superior del bbox (30% inferior)

def visualize_detection_on_frame(frame_data, polygons, bbox_list, detection_points, camera_id, zone_names):
    """
    Crea una imagen de visualización que muestra las zonas y vehículos sobre la imagen real
    
    Args:
        frame_data: Bytes de la imagen o imagen en formato numpy array
        polygons: Lista de polígonos en coordenadas absolutas
        bbox_list: Lista de bounding boxes de vehículos (x1, y1, x2, y2)
        detection_points: Lista de puntos (x, y) de detección
        camera_id: ID de la cámara
        zone_names: Nombres de las zonas asociadas a los polígonos
    
    Returns:
        str: Ruta del archivo de imagen guardado
    """
    # Verificar si frame_data es bytes o base64
    if isinstance(frame_data, bytes) or isinstance(frame_data, str):
        try:
            # Si es base64, decodificar
            if isinstance(frame_data, str):
                if frame_data.startswith(('data:image', 'base64')):
                    # Eliminar prefijo si existe
                    if ',' in frame_data:
                        frame_data = frame_data.split(',')[1]
                frame_data = base64.b64decode(frame_data)
            
            # Convertir bytes a matriz numpy
            nparr = np.frombuffer(frame_data, np.uint8)
            img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
            
            if img is None:
                raise ValueError("No se pudo decodificar la imagen")
        except Exception as e:
            print(f"Error al decodificar la imagen: {str(e)}")
            # Crear imagen en blanco si falla la decodificación
            img = np.ones((1080, 1920, 3), dtype=np.uint8) * 255
    elif isinstance(frame_data, np.ndarray):
        # Ya es una matriz numpy
        img = frame_data.copy()
    else:
        # Si no hay imagen o tipo no compatible, crear una en blanco
        img = np.ones((1080, 1920, 3), dtype=np.uint8) * 255
    
    # Asegurar que la imagen es 3 canales (BGR)
    if len(img.shape) == 2:  # Si es grayscale
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    
    height, width = img.shape[:2]
    
    # Colores para las zonas (en formato BGR)
    colors = {
        'restricted': (0, 0, 255),    # Rojo para zonas restricted
        'parking': (0, 255, 0),       # Verde para zonas parking
        'other': (255, 0, 0)          # Azul para otras zonas
    }
    
    # Crear una copia para dibujar con transparencia
    overlay = img.copy()
    
    # Dibujar cada polígono
    for i, polygon in enumerate(polygons):
        # Determinar color basado en el nombre de la zona
        zone_name = zone_names[i] if i < len(zone_names) else "other"
        if "restricted" in zone_name.lower():
            color = colors['restricted']
        elif "parking" in zone_name.lower():
            color = colors['parking']
        else:
            color = colors['other']
        
        # Convertir a formato de puntos para OpenCV
        pts = np.array(polygon, np.int32)
        pts = pts.reshape((-1, 1, 2))
        
        # Dibujar polígono relleno en la capa overlay
        cv2.fillPoly(overlay, [pts], color)
        
        # Dibujar borde del polígono en la imagen original
        cv2.polylines(img, [pts], True, color, 2)
        
        # Añadir etiqueta de la zona
        centroid = np.mean(polygon, axis=0, dtype=np.int32)
        cv2.putText(img, zone_name, (int(centroid[0]), int(centroid[1])), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(img, zone_name, (int(centroid[0]), int(centroid[1])), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
    
    # Combinar overlay con transparencia
    alpha = 0.3  # Transparencia
    cv2.addWeighted(overlay, alpha, img, 1 - alpha, 0, img)
    
    # Dibujar bounding boxes y puntos de detección de los vehículos
    if bbox_list and detection_points:
        # Asegurarnos de que sean listas aunque solo haya un elemento
        if not isinstance(bbox_list[0], (list, tuple)):
            bbox_list = [bbox_list]
        if not isinstance(detection_points[0], (list, tuple)):
            detection_points = [detection_points]
            
        for i, (bbox, point) in enumerate(zip(bbox_list, detection_points)):
            if len(bbox) == 4:  # Verificar formato de bbox
                x1, y1, x2, y2 = [int(c) for c in bbox]
                
                # Dibujar bounding box
                cv2.rectangle(img, (x1, y1), (x2, y2), (255, 255, 0), 2)
                
                # Dibujar ID del objeto
                cv2.putText(img, f"Obj {i+1}", (x1, y1 - 10), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1, cv2.LINE_AA)
                
                # Dibujar punto de detección
                x, y = point
                # Círculo grande amarillo
                cv2.circle(img, (int(x), int(y)), 8, (0, 255, 255), -1)
                # Círculo pequeño negro (para contraste)
                cv2.circle(img, (int(x), int(y)), 3, (0, 0, 0), -1)
    
    # Añadir timestamp e info de la cámara
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cv2.putText(img, f"Camera: {camera_id} | Time: {timestamp}", 
               (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(img, f"Camera: {camera_id} | Time: {timestamp}", 
               (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1, cv2.LINE_AA)
    
    # Guardar la imagen
    filename = f"{DEBUG_DIR}/cam_{camera_id}_{int(time.time())}.jpg"
    cv2.imwrite(filename, img)
    
    print(f"Debug image saved: {filename}")
    return filename


def point_in_polygon(point, polygon):
    """Implementación del algoritmo ray-casting para determinar si un punto está dentro de un polígono"""
    x, y = point
    n = len(polygon)
    inside = False
    p1x, p1y = polygon[0]
    for i in range(n + 1):
        p2x, p2y = polygon[i % n]
        if y > min(p1y, p2y):
            if y <= max(p1y, p2y):
                if x <= max(p1x, p2x):
                    if p1y != p2y:
                        xinters = (y - p1y) * (p2x - p1x) / (p2y - p1y) + p1x
                    if p1x == p2x or x <= xinters:
                        inside = not inside
        p1x, p1y = p2x, p2y
    return inside

def get_zones_from_redis(camera_id):
    """
    Obtiene las zonas desde Redis para una cámara específica usando la estructura hash
    
    Args:
        camera_id: ID de la cámara
    
    Returns:
        dict: Diccionario con las zonas de la cámara o {} si no existen
    """
    try:
        redis_conn = redis.Redis(connection_pool=redis_pool)
        
        # Obtenemos el valor del hash para esta cámara del contenedor "restricted_area"
        zone_data_str = redis_conn.hget(REDIS_ZONES_CONTAINER, camera_id)
        
        if not zone_data_str:
            logger.warning(f"No se encontraron zonas en Redis para la cámara {camera_id}")
            return {}
            
        # Convertir de string JSON a diccionario Python
        zone_data = json.loads(zone_data_str)
        logger.debug(f"Zonas obtenidas de Redis para cámara {camera_id}: {zone_data}")
        
        return zone_data
    except json.JSONDecodeError as e:
        logger.error(f"Error al decodificar JSON de Redis para cámara {camera_id}: {str(e)}")
        return {}
    except Exception as e:
        logger.error(f"Error al obtener zonas desde Redis para cámara {camera_id}: {str(e)}")
        return {}

def parse_zones_and_polygons(zone_data, img_shape=None):
    """
    Procesa zone_restricted y convierte coordenadas relativas a absolutas
    Versión actualizada para manejar sub-zonas con minimum_time individual
    Args:
        zone_data: Diccionario con las zonas
        img_shape: Tuple (height, width) de la imagen
    Returns:
        (list) zonas válidas, (list) polígonos válidos en coordenadas absolutas,
        (dict) tiempos mínimos asociados a cada zona
    """

    if not zone_data or not isinstance(zone_data, dict):
        return [], [], {}

    try:
        zones = []
        all_polygons = []
        minimum_times = {}  # Diccionario para almacenar minimum_time por zona
        target_zones = {'restricted', 'parking'}  # Usamos set para búsqueda más rápida

        # Verificar shape de imagen
        if img_shape is None or len(img_shape) < 2:
            logger.warning("No se proporcionó img_shape válido, usando valores por defecto 1080x1920")
            img_shape = (1080, 1920)  # Valor por defecto HD

        img_height, img_width = img_shape[0], img_shape[1]

        # Procesar cada tipo de zona (restricted, parking)
        for zone_type, zone_info in zone_data.items():
            if zone_type.lower() in target_zones and isinstance(zone_info, dict):
                # Procesar cada sub-zona
                for sub_zone_name, sub_zone_info in zone_info.items():
                    if isinstance(sub_zone_info, dict) and 'coords' in sub_zone_info:
                        # Es una sub-zona válida con coordenadas
                        coords = sub_zone_info.get('coords')
                        zone_name = f"{zone_type}_{sub_zone_name}"  # Nombre combinado: "restricted_zona1"
                        
                        # Extraer minimum_time específico de esta sub-zona
                        min_time = sub_zone_info.get('minimum_time', 0)
                        minimum_times[zone_name] = min_time
                        
                        zones.append(zone_name)
                        
                        # Procesar coordenadas
                        if coords:
                            if coords and all(isinstance(p, (list, tuple)) and len(p) >= 2 for p in coords[0]):
                                # Es una lista de polígonos (coords es [[[x,y],...], [[x,y],...]])
                                polygon_list = coords
                            else:
                                # Es un solo polígono (coords es [[x,y],...])
                                polygon_list = [coords]
                            
                            # Procesar cada polígono individualmente
                            for poly in polygon_list:
                                if isinstance(poly, list) and len(poly) >= 3:
                                    absolute_poly = []
                                    for point in poly:
                                        if isinstance(point, (list, tuple)) and len(point) == 2:
                                            try:
                                                x_rel, y_rel = float(point[0]), float(point[1])
                                                # Conversión a coordenadas absolutas
                                                x_abs = x_rel * img_width
                                                y_abs = y_rel * img_height
                                                absolute_poly.append((x_abs, y_abs))
                                            except (ValueError, TypeError) as e:
                                                logger.warning(f"Punto inválido en zona {zone_name}: {point} - {str(e)}")
                                                continue
                                    
                                    # Validación de polígono mínimo (3 puntos)
                                    if len(absolute_poly) >= 3:
                                        all_polygons.append(absolute_poly)
                                    else:
                                        logger.warning(f"Polígono en zona {zone_name} no tiene suficientes puntos válidos")
                    
                    # Compatibilidad con el formato antiguo (sin sub-zonas)
                    elif sub_zone_name == 'coords' and isinstance(sub_zone_info, list):
                        # Es el formato antiguo - coordenadas directas
                        coords = sub_zone_info
                        zones.append(zone_type)
                        
                        # Para el formato antiguo, buscamos minimum_time en el nivel del tipo de zona
                        min_time = zone_info.get('minimum_time', 0)
                        minimum_times[zone_type] = min_time
                        
                        # Procesar polígono
                        if isinstance(coords, list) and len(coords) >= 3:
                            absolute_poly = []
                            for point in coords:
                                if isinstance(point, (list, tuple)) and len(point) == 2:
                                    try:
                                        x_rel, y_rel = float(point[0]), float(point[1])
                                        x_abs = x_rel * img_width
                                        y_abs = y_rel * img_height
                                        absolute_poly.append((x_abs, y_abs))
                                    except (ValueError, TypeError) as e:
                                        logger.warning(f"Punto inválido en zona {zone_type}: {point} - {str(e)}")
                                        continue
                            
                            if len(absolute_poly) >= 3:
                                all_polygons.append(absolute_poly)
                            else:
                                logger.warning(f"Polígono en zona {zone_type} no tiene suficientes puntos válidos")

        logger.debug(f"Zonas parseadas: {zones} | Polígonos válidos: {len(all_polygons)} | Tiempos mínimos: {minimum_times}")
        return zones, all_polygons, minimum_times

    except Exception as e:
        logger.error(f"Error parsing zone_restricted: {str(e)}")
        return [], [], {}

def en_area_restringida_math(point, polygons):
    """Determina si el punto central está dentro de cualquier polígono restringido"""
    x, y = point
    for polygon in polygons:
        if point_in_polygon((x, y), polygon):
            return True
    return False

def get_central_detection_point(bbox, horizontal_offset=POINT_HORIZONTAL_OFFSET, vertical_position=POINT_VERTICAL_POSITION):
    """Genera un punto central de detección en posición relativa al bbox original"""
    try:
        x1, y1, x2, y2 = bbox
        width = x2 - x1
        height = y2 - y1
        
        # Posición vertical (porcentaje desde la parte superior del bbox)
        y_pos = y1 + int(height * vertical_position)
        
        # Posición horizontal (POINT_HORIZONTAL_OFFSET ahora controla la posición central)
        # 0.5 = centro exacto, 0.0 = extremo izquierdo, 1.0 = extremo derecho
        x_pos = x1 + int(width * (0.5 + (horizontal_offset - 0.5)))
        
        # Asegurar que el punto esté dentro de los límites de la imagen
        central_point = (
            max(0, min(x_pos, CAMERA_RESOLUTION[0]-1)), 
            max(0, min(y_pos, CAMERA_RESOLUTION[1]-1))
        )
        
        return central_point
    except Exception as e:
        logger.error(f"Error calculando punto central de detección: {str(e)}")
        return ((x1 + x2) // 2, y2)  # Fallback a centro de la base

def get_frame_from_redis(data):
    """Obtiene el frame original desde Redis"""
    try:
        r = redis.Redis(connection_pool=redis_pool, decode_responses=False)
        image_bytes = r.get(f"{data[DATA_CAMERA]}{data[DATA_EPOCH_FRAME]}")
        if not image_bytes:
            logger.error(f"Frame {data[DATA_EPOCH_FRAME]} no existe en Redis")
            return None
        np_arr = np.frombuffer(image_bytes, dtype=np.uint8)
        frame = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)
        return frame
    except Exception as e:
        logger.error(f"Error al obtener frame de Redis: {str(e)}")
        return None

def procesamiento_coordenadas_sync(data, publish_callback):
    """Función principal de procesamiento - Con visualización usando frames de Redis"""
    start_time = time.time()
    cam_id = data.get(DATA_CAMERA, "unknown")
    current_time = time.time()

    try:
        # NUEVO: Obtener dimensiones de la imagen
        img_shape = data.get("shape", [1080, 1920])  # [height, width] por defecto HD
        if len(img_shape) >= 2:
            img_shape = (int(img_shape[0]), int(img_shape[1]))
        else:
            logger.warning(f"Cámara {cam_id} - shape inválido, usando 1080x1920")
            img_shape = (1080, 1920)
        
        # Obtener frame desde Redis usando la función específica
        frame = get_frame_from_redis(data)
        if frame is not None:
            logger.info(f"Frame obtenido desde Redis para cámara {cam_id}")
        else:
            logger.warning(f"No se pudo obtener frame desde Redis para cámara {cam_id}")
        
        # MODIFICADO: Obtener las zonas desde Redis en lugar del mensaje RabbitMQ
        zone_data = get_zones_from_redis(cam_id)
        
        if not isinstance(zone_data, dict):
            logger.debug(f"Cámara {cam_id} - Formato de zonas inválido en Redis, descartando")
            return
            
        # 2. Verificar presencia de zonas objetivo ANTES de procesar
        target_zones_present = any(
            zone.lower() in {'restricted', 'parking'} 
            for zone in zone_data.keys()
        )
        
        if not target_zones_present:
            logger.debug(f"Cámara {cam_id} - No contiene zonas 'restricted' o 'parking' en Redis, descartando")
            return

        # 3. Parsear zonas y polígonos (MODIFICADO para obtener también minimum_time)
        zone_ids, polygons, minimum_times = parse_zones_and_polygons(zone_data, img_shape=img_shape)
        
        if not zone_ids:  # Verificación redundante por seguridad
            logger.debug(f"Cámara {cam_id} - No se encontraron zonas válidas después del parseo")
            return

        # 4. Crear estructura de zonas con sus polígonos
        zone_polygons = {
            zone_id: [polygon]  
            for zone_id, polygon in zip(zone_ids, polygons)
            # No filtramos por 'restricted' o 'parking' ya que ahora tenemos nombres compuestos
        }

        # 5. Estructura de salida (solo si hay zonas válidas)
        output = {
            "camera_id": cam_id,
            "epoch_frame": data.get(DATA_EPOCH_FRAME, 0),
            "n_objects": 0,
            "zone_restricted": zone_data,  # Incluimos las zonas completas obtenidas de Redis
            "object_dict": {zone_id.lower(): [] for zone_id in zone_polygons}  # Solo zonas filtradas
        }

        total_objects_in_zones = 0
        
        # Preparar listas para acumular todos los vehículos y puntos
        all_bboxes = []
        all_detection_points = []

        # 6. Procesar cada objeto
        for category, objects in data.get(DATA_OBJETOS_DICT, {}).items():
            #output["object_dict"][category] = []
            
            for obj in objects:
                # 6.1. Obtener coordenadas del bbox original
                coords = obj.get(DATA_OBJETO_COORDENADAS, (0, 0, 0, 0))
                
                # NUEVO: Validación adicional de coordenadas
                if len(coords) != 4 or any(not isinstance(c, (int, float)) for c in coords):
                    logger.warning(f"Objeto inválido en cámara {cam_id}: coordenadas malformadas")
                    continue
                
                # Añadir bbox a la lista acumulada
                all_bboxes.append(coords)
                
                # 6.2. Obtener los puntos de detección
                detection_points = get_central_detection_point(coords)
                
                # Añadir punto de detección a la lista acumulada
                all_detection_points.append(detection_points)
                
                # 6.3. Verificar si alguno de los puntos está en zona restringida
                detected_zones = [
                    zone_id
                    for zone_id, zone_polygons in {
                        z_id: [poly] for z_id, poly in zip(zone_ids, polygons)
                    }.items()
                    if en_area_restringida_math(detection_points, zone_polygons)
                ]

                # Solo procesar si está en al menos una zona
                if detected_zones:
                    total_objects_in_zones += 1
                    zone_key = detected_zones[0].lower()  # Usamos la primera zona como categoría
                
                    # Lógica de alertas
                    alert = None
                    tracking_id = obj.get(DATA_TRAKING_ID)

                    if tracking_id:
                        if tracking_id not in vehicle_timers.get(cam_id, {}):
                            vehicle_timers.setdefault(cam_id, {})[tracking_id] = current_time
                            
                            alert = {
                                "type": "Ingreso",
                                "message": f"Ingreso detectado en zona(s): {', '.join(detected_zones)}"
                            }

                            logger.info(f"INGRESO DETECTADO: {alert} ")

                        else:
                            elapsed = current_time - vehicle_timers[cam_id][tracking_id]
                            min_time = minimum_times.get(zone_key, PERMANENCE_THRESHOLD * 1000)  # Obtener minimum_time o usar el valor por defecto
                            min_time_sec = min_time / 1000  # Convertir a segundos si está en milisegundos
                            
                            if elapsed > min_time_sec:
                                vehicle_timers[cam_id][tracking_id] = current_time  # Reset timer
                                logger.info(f"PERMANENCIA DETECTADA")
                
                    # 6.4. Construir objeto final
                    output["object_dict"][zone_key].append({
                        **{k: v for k, v in obj.items() if k != DATA_OBJETO_COORDENADAS},
                        "coords": coords,
                    })

        # Actualizar conteo real de objetos
        output["n_objects"] = total_objects_in_zones
        
        # DEBUG: Generar una visualización con todos los vehículos y zonas en la imagen original
        if frame is not None:
            debug_img = visualize_detection_on_frame(
                frame_data=frame,
                polygons=polygons,
                bbox_list=all_bboxes,
                detection_points=all_detection_points,
                camera_id=cam_id,
                zone_names=zone_ids
            )
            
            logger.info(f"Generada imagen de depuración para cámara {cam_id} - Vehículos: {len(all_bboxes)} - Ruta: {debug_img}")
        else:
            logger.warning(f"No se pudo generar imagen de depuración para cámara {cam_id} - No hay datos de imagen en Redis")

        # 7. Publicar SOLO si hay zonas de interés válidas
        publish_callback(output)

        if total_objects_in_zones > 0:
            logger.info(f"Cámara {cam_id} - Publicados {total_objects_in_zones} objetos en zonas")
        else:
            logger.info(f"Cámara {cam_id} - Zonas vacías publicadas")

    except Exception as e:
        logger.error(f"Error en cámara {cam_id}: {str(e)}", exc_info=True)
    finally:
        logger.info(f"Tiempo total: {time.time() - start_time}s")