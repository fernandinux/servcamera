...
class VehicleDetector:
    def __init__(self, interval_minutes=5, distance_threshold=15, execute_duration=3, 
                 tiempo_considerado_abandono=10, similarity_threshold=0.85,
        ...
        # Registro principal de vehículos - MODIFICADO para almacenar más información
        # {camera: {id: {
        #   "centroid": (x, y), 
        #   "last_seen": timestamp,
        #   "first_seen": timestamp,
        #   "abandoned_time": tiempo_acumulado,
        #   "coords": [x1,y1,x2,y2], 
        #   "type": vehicle_type,
        #   "reported": False/True
        # }}}
        self.vehicle_registry = defaultdict(dict)
    ...
    def process(self, camera_id, object_dict):
            ...
            if vehicle_data["abandoned_time"] >= self.tiempo_considerado_abandono:
                ...
                if not vehicle_data["reported"] and not self._is_duplicate_report(camera_id, current_id, 
                    ...
                    logger.info(f"[ABANDONO] Vehículo {current_id} tipo {vehicle_type} abandonado: "
                               f"Tiempo en posición: {vehicle_data['abandoned_time']:.2f}s, "
                               f"Tiempo total detectado: {total_time_detected:.2f}s")
        ...
        logger.info(f"[RESUMEN] Procesados {len(vehicles)} vehículos, {len(abandoned)} considerados abandonados")
