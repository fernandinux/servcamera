...
POINT_RADIUS = 5  # Radio en píxeles para la detección del punto
UPPER_PERCENTAGE = 0.3  # Porcentaje de la altura del bbox ajustado donde colocaremos el punto
POINT_COLOR = (0, 255, 255)  # Color amarillo para el punto de detección
BBOX_COLOR = (0, 255, 0)     # Color verde para el bounding box
ADJUSTED_BBOX_COLOR = (255, 0, 0)  # Color azul para el bbox ajustado
POLYGON_COLOR = (0, 0, 255)  # Color rojo para los polígonos restringidos
...
def guardar_imagen(frame, tracking_id):
    ...
    try:
        ...
        filename = f"{DEBUG_IMG_DIR}/doblelinea_{tracking_id}_{int(time.time())}.jpg"
...
def dibujar_visualizacion(frame, polygons, tracking_id, bbox, adjusted_bbox, detection_point):
    """Dibuja las detecciones en el frame para cruce en doble línea"""
    try:
        # Dibujar área restringida (polígonos)
        for poligono in polygons:
            puntos = np.array(poligono, dtype=np.int32).reshape((-1, 1, 2))
            cv2.polylines(frame, [puntos], isClosed=True, color=POLYGON_COLOR, thickness=2)
        
        # Dibujar bounding box completo (verde)
        x1, y1, x2, y2 = bbox
        cv2.rectangle(frame, (x1, y1), (x2, y2), BBOX_COLOR, 2)
        
        # Dibujar bounding box ajustado (azul)
        adj_x1, adj_y1, adj_x2, adj_y2 = adjusted_bbox
        cv2.rectangle(frame, (adj_x1, adj_y1), (adj_x2, adj_y2), ADJUSTED_BBOX_COLOR, 2)
        
        # Dibujar punto de detección (amarillo)
        cv2.circle(frame, detection_point, POINT_RADIUS, POINT_COLOR, -1)
        ...
        cv2.putText(frame, "Punto deteccion", (detection_point[0]+10, detection_point[1]), 
...
def get_midpoint_in_adjusted_bbox(adjusted_bbox, upper_percentage=UPPER_PERCENTAGE):
    """Calcula un punto en la parte superior del bounding box ajustado"""
    try:
        x1, y1, x2, y2 = adjusted_bbox
        height = y2 - y1
        # Calculamos el punto en la parte superior del bbox ajustado
        y_point = y1 + int(height * upper_percentage)
        x_point = (x1 + x2) // 2  # Punto medio en el eje X
        
        # Aseguramos que las coordenadas estén dentro de los límites
        x_point = max(0, min(x_point, CAMERA_RESOLUTION[0]))
        y_point = max(0, min(y_point, CAMERA_RESOLUTION[1]))
        
        return (x_point, y_point)
    except Exception as e:
        logger.error(f"Error calculando punto en bbox ajustado: {str(e)}")
        return ((x1 + x2) // 2, y1)  # Valor por defecto si hay error

def en_area_restringida(point, mask, radius=POINT_RADIUS):
    """Verifica si un punto (con cierto radio) está dentro del área restringida"""
    try:
        x, y = point
        # Asegurar que las coordenadas estén dentro de los límites de la imagen
        x, y = max(0, x), max(0, y)
        
        # Crear una pequeña ROI alrededor del punto
        x1, y1 = max(0, x - radius), max(0, y - radius)
        x2, y2 = min(mask.shape[1], x + radius + 1), min(mask.shape[0], y + radius + 1)
        
        if x1 >= x2 or y1 >= y2:  # ROI inválida
            return False
            
        roi = mask[y1:y2, x1:x2]
        return cv2.countNonZero(roi) > 0
    except Exception as e:
        logger.error(f"Error verificando área restringida para punto: {str(e)}")
        return False

def procesamiento_coordenadas(data, publish_callback):
    """Función principal de procesamiento para detección de cruce en doble línea"""
    ...
    try:
        ...
        for categoria, objetos in data.get(DATA_OBJETOS_DICT, {}).items():
            for obj in objetos:
                ...
                # 6. Proceso en dos pasos:
                # a) Primero ajustamos el bbox a la parte inferior
                coords = obj.get(DATA_OBJETO_COORDENADAS, (0, 0, 0, 0))
                adjusted_coords = adjust_coords_for_lower_part(coords)
                
                # b) Luego calculamos el punto en la parte superior del bbox ajustado
                detection_point = get_midpoint_in_adjusted_bbox(adjusted_coords)
                
                if en_area_restringida(detection_point, mask):
                    ...
                    if tracking_id not in vehicle_timers[cam_id]:
                        ...
                        alert = {
                            'camera_id': cam_id,
                            'category': 1,
                            'message': 'Cruce de Vehiculo en doble linea detectado',
                            'type': 1,
                            'sub_type': 1,
                    ...
                    # Manejo de alertas de permanencia
                        ...
                        if elapsed_time > PERMANENCE_THRESHOLD:
                            alert = {
                                'camera_id': cam_id,
                                'category': 1,
                                'message': f'Permanencia de Vehículo en doble linea: {elapsed_time:.2f}s',
                                'type': 1,
                                'sub_type': 2,
                                'epoch_object': obj.get(DATA_OBJETO_EPOCH_OBJECT, 0),
                                'epoch_frame': data.get(DATA_EPOCH_FRAME, 0),
