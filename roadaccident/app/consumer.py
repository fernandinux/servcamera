...
class ExampleConsumer(object):
    ...
    def __init__(self, amqp_url, message_callback=None, prefetch_count=10):
    ...
    def purge_queue(self):
        logger.info('Purging queue: %s', self.QUEUE)
        # Se asume que el canal ya está abierto
        self._channel.queue_purge(queue=self.QUEUE, callback=self.on_purge_ok)

    def on_purge_ok(self, method_frame):
        logger.info('Queue purged: %s, mensajes eliminados: %s',
                    self.QUEUE, method_frame.method.message_count)
        
class ReconnectingExampleConsumer(object):
    """Consumer que se reconecta si es necesario."""
    def __init__(self, amqp_url, message_callback=None, prefetch_count=1):
        self._prefetch_count = prefetch_count
